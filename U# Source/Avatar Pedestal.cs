...
public class AvatarPedestal : UdonSharpBehaviour
{
    ...
    public void OnPlayerCollisionEnter(VRCPlayerApi player) { if (Detect_Player && Event_OnCollisionEnter && player.isLocal) { SendCustomEvent("Run"); } }
    public void OnPlayerCollisionExit(VRCPlayerApi player) { if (Detect_Player && Event_OnCollisionExit && player.isLocal) { SendCustomEvent("Run"); } }
    public void OnPlayerTriggerEnter(VRCPlayerApi player) { if (Detect_Player && Event_OnTriggerEnter && player.isLocal) { SendCustomEvent("Run"); } }
    public void OnPlayerTriggerExit(VRCPlayerApi player) { if (Detect_Player && Event_OnTriggerExit && player.isLocal) { SendCustomEvent("Run"); } }

    public bool Event_OnPickup = false;
    public bool Event_OnDrop = false;
    void OnPickup() { if (Event_OnPickup) { SendCustomEvent("Run"); } }
    void OnDrop() { if (Event_OnDrop) { SendCustomEvent("Run"); } }

    public bool Event_OnPickupUseDown = false;
    public bool Event_OnPickupUseUp = false;
    void OnPickupUseDown() { if (Event_OnPickupUseDown) { SendCustomEvent("Run"); } }
    void OnPickupUseUp() { if (Event_OnPickupUseUp) { SendCustomEvent("Run"); } }
    ...
    public void Run()
    {
        if(Networking.LocalPlayer==null)
        {
            ...
            else
            {
                ...
                {
                    if (Avatars[i] == Pedestals[0].blueprintId)
                    {
                        Debug.Log("Udon Toolbox: Player changed avatar, used avatar in Element:" + i.ToString(), this);
                    }
                }
            }
        }
        ...
        {
            Pedestals[0].SetAvatarUse(Networking.LocalPlayer);
        }
    }
